...
def getSection(inf: WinINF, section: str):
    ...
    for sec in inf:
        ...
        if sec.GetName().lower().strip() == toFind:
            print(f'{inf.GetFileName()}: {sec.GetName()} matches {toFind}')
            return sec
    return None
...
# Appends a striing to a string list if not present already
def appendIfNew(strList: list[str], value: str):
    if value not in strList:
        strList.append(value)

# gets a liist of all CopyFiiles sections. Direct file references (@<something>) are appended to rawFiles.
...
def getAllFilesFromSections(inf: WinINF, sectionNames: list[str], knownFiles: list[str]):
    for secName in sectionNames:
        ...
        for key, value, comment in section:
            if (key):
                ...
                appendIfNew(knownFiles, fname.strip())

# Get a list of all files referenced by the SourceDisksFiles section of this INF
def getSourceDisksFiles(inf: WinINF, knownFiles: list[str]):
    ...
    for key, value, comment in sdfSection:
        if (key):
            appendIfNew(knownFiles, key.strip())
...
def writeSourceDisksNamesAndFiles(inf: WinINF, knownFiles: list[str], cabName: str):
...
def handleInf(filename: str, outInfName: str, cabName: str, filesInDirectory: list[str]) -> bool:
    inf = WinINF()
    ...
    filesInDirectory += knownFiles

    return True

# Checks if a string list contains an item, case insensiitively
...
def copyFilesAndWriteCab(sourceDir: str, outDir: str, outCab: str, win98files: list[str], filesInDirectory: list[str]) -> bool:
    cabArchive = CabArchive()

    for thisFile in filesInDirectory:
        isFromWin98 = containsCaseInsensitive(win98files, thisFile)
        localName = getFilenameCaseInsensitive(sourceDir, thisFile)
        ...
        if localName:
            ...
            cabArchive[thisFile] = CabFile(fileData)
        elif isFromWin98:
            logi(f'File {thisFile} supplied by Win9x!')
        else:
            ...
            if localCompressed:
                logi(f'File {thisFile} was SZDD compressed, decompressing...')
                ...
                loge(f'File {thisFile} is NOT supplied by driver nor Win9x itself!')
                raise Exception(f'Please fix {thisFile} before packaging ISO!')
...
# Scans a sub directory and plugs all its INF files iinto the INF handler
